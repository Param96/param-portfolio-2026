    ...
    <section className="relative w-full py-32 bg-transparent text-[var(--text-main)] overflow-hidden border-t border-[var(--text-main)]/10">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="mb-20 flex flex-col items-center text-center">
          ...
          <h2 className="text-3xl md:text-5xl font-light tracking-tight text-[var(--text-main)]">
            Public <span className="font-serif italic text-[var(--text-main)]/60">Repositories.</span>
          </h2>
          <p className="mt-4 text-[var(--text-main)]/60 max-w-2xl font-inter text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {repos.map((repo, idx) => (
            <motion.div
              ...
              key={repo.id}
              className="group bg-[var(--text-main)]/[0.03] border border-[var(--text-main)]/10 rounded-xl p-6 flex flex-col hover:border-[#00FF41]/40 hover:bg-[var(--text-main)]/[0.08] transition-all duration-300 relative overflow-hidden"
            >
              {/* Top Accent */}
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-2">
                  <Code2 className="w-5 h-5 text-[var(--text-main)]/60 group-hover:text-[#00FF41] transition-colors" />
                  <h3 className="font-semibold text-lg text-[var(--text-main)] truncate max-w-[200px]" title={repo.name}>
                    {repo.name}
                  </h3>
                </div>
                <a 
                  ...
                  className="text-[var(--text-main)]/60 hover:text-[#00FF41] transition-colors"
                >
                  <ExternalLink className="w-5 h-5" />
                </a>
              </div>

              <p className="text-[var(--text-main)]/80 text-sm mb-6 flex-grow line-clamp-3 leading-relaxed">
                {repo.description}
              </p>

              <div className="flex items-center justify-between mt-auto pt-4 border-t border-[var(--text-main)]/10">
                <div className="flex items-center gap-4 text-xs font-mono text-[var(--text-main)]/60">
                  {repo.language && (
